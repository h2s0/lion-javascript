...
// 바로 실행되는게 아닌, 사용자가 어떤 인터렉션(행동)을 했을 때 코드 로직이 실행되는
...
//     if(!value){
//         throw new TypeError('Type is not correct')
//     }

// }catch(error){
//     console.log(error.name);                // 에러 이름
//     console.log(error.message);             // 에러 메세지
//     console.log(error.stack);               // 전체 에러코드
// }

    // try 절에 나온 오류들은 다 catch 절에 나오게 된다.
...
let data = JSON.stringify({
  ...
  try{
    ...
    confirm ('해당 데이터를 불러오는데 실패하였습니다. 다시 시도할까요?'); // 예외 처리
  }